...
class ArchQueen(Troop):
    ...
    def attack(self, huts, Townhall, walls,cannons, wizTowers, LMov):
        ...
        if(LMov=="right"):
            ...
            for wall in walls:
                if(wall.row<= self.r+2 and wall.row>=self.r-6 and wall.column>=self.c-3 and wall.column<=self.c+5 and wall.health>0):
                    wall.health=wall.health -50
            for hut in huts:
                if(doOverlap(self.r-6,self.c-3,self.r+2,self.c+5,hut.UpperLeftRow,hut.UpperLeftColumn,hut.LowerRightRow,hut.LowerRightColumn )and hut.health>0):
                    hut.health=hut.health -50
            for cannon in cannons:
                if(doOverlap(self.r-6,self.c-3,self.r+2,self.c+5,cannon.UpperLeftRow,cannon.UpperLeftColumn,cannon.LowerRightRow,cannon.LowerRightColumn) and cannon.health>0):
                    cannon.health=cannon.health -50
            for wiz in wizTowers:
                if(doOverlap(self.r-6,self.c-3,self.r+2,self.c+5,wiz.UpperLeftRow,wiz.UpperLeftColumn,wiz.LowerRightRow,wiz.LowerRightColumn) and wiz.health>0):
                    wiz.health=wiz.health -50
            if(doOverlap(self.r-6,self.c-3,self.r+2,self.c+5,Townhall.UpperLeftRow,Townhall.UpperLeftColumn,Townhall.LowerRightRow,Townhall.LowerRightColumn)and Townhall.health>0):
                Townhall.health = Townhall.health - 50
        elif(LMov=="down"):
            for wall in walls:
                if(wall.row<= self.r+12 and wall.row>=self.r+4 and wall.column>=self.c-3 and wall.column<=self.c+5 and wall.health>0):
                    wall.health=wall.health -50
            for hut in huts:
                if(doOverlap(self.r+4,self.c-3,self.r+12,self.c+5,hut.UpperLeftRow,hut.UpperLeftColumn,hut.LowerRightRow,hut.LowerRightColumn )and hut.health>0):
                    hut.health=hut.health -50
            for cannon in cannons:
                if(doOverlap(self.r+4,self.c-3,self.r+12,self.c+5,cannon.UpperLeftRow,cannon.UpperLeftColumn,cannon.LowerRightRow,cannon.LowerRightColumn) and cannon.health>0):
                    cannon.health=cannon.health -50
            for wiz in wizTowers:
                if(doOverlap(self.r+4,self.c-3,self.r+12,self.c+5,wiz.UpperLeftRow,wiz.UpperLeftColumn,wiz.LowerRightRow,wiz.LowerRightColumn) and wiz.health>0):
                    wiz.health=wiz.health -50
            if(doOverlap(self.r+4,self.c-3,self.r+12,self.c+5,Townhall.UpperLeftRow,Townhall.UpperLeftColumn,Townhall.LowerRightRow,Townhall.LowerRightColumn)and Townhall.health>0):
                Townhall.health = Townhall.health - 50
